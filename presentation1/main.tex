...
\frametitle{Skeletal Constraints}
  \begin{items}
    \begin{oitem}{\litem}
      ...
      \sitem {\footnotesize Pseudo-observation merged into each update step with constant value of 0.}
    \end{oitem}
    \begin{columns}[c]
...
\frametitle{Learning Parameters from Data}
  \begin{items}
    \begin{oitem}{\litem}
      ...
      \sitem {\footnotesize Training is done using the ground-truth annotated CMU Panoptic dataset.} \\
      \sitem {\footnotesize Sequences reserved for testing are excluded from the training.}
    \end{oitem}
    \input{tikzfigures/learning.tex}
  \end{items}
\end{frame}
...
\frametitle{Learning Parameters from Data -- Custom YOLO Head}
  \begin{items}
    \begin{oitem}{\litem}
      Summary: \\
      \sitem {\footnotesize Standard YOLO does not provide uncertainty output and does not predict invisible joints.} \\
      \sitem {\footnotesize Dataset generated from CMU Panoptic by training random camera view every second.} \\
      \sitem {\footnotesize Custom head outputs $x$ and $y$ offsets as well as covariance matrix.} \\
      \sitem {\footnotesize Final layer of head is initialized to zero to start training with reasonable values.} \\
      \sitem {\footnotesize Extensive data augmentation is performed to avoid overfitting.}
    \end{oitem}
    \input{tikzfigures/learning_yolo.tex}
  \end{items}
\end{frame}
...
\frametitle{Learning Parameters from Data -- Kalman Filter}
  \begin{items}
    \begin{oitem}{\litem}
      Summary: \\
      \sitem {\footnotesize Used only the annotations of the CMU Panoptic dataset.} \\
      \sitem {\footnotesize Dataset generated by extracting individual persons tracks from annotations.} \\
      \sitem {\footnotesize Learns all the parameters of the Kalman filter such as dynamics and constraints.} \\
      \sitem {\footnotesize Training by taking 100 frame segments of the ground truth tracks.} \\
      \sitem {\footnotesize Then generating synthetic observations, running filter, and minimizing MSE.}
    \end{oitem}
    \input{tikzfigures/learning_kalman.tex}
  \end{items}
\end{frame}
...
\frametitle{Experimental Setup}
  \begin{items}
    \begin{oitem}{\litem}
      ...
      \sitem {\footnotesize Evaluate using different configurations of the tracking system.} \\
...
\frametitle{Quantitative Results -- Performance vs. Model Size}
  \begin{items}
    \begin{oitem}{\litem}
      Main Finding: \\
      \sitem {\footnotesize Larger models only moderately improve performance.} \\
      \sitem {\footnotesize Largest performance jump is observed between the small and medium models.} \\
      \sitem {\footnotesize Looking at MPJPE there is no clear advantage for larger models.}
    \end{oitem}
    \begin{widepage}
      \begin{figure}
        \centering
...
\frametitle{Quantitative Results -- Skeletal Constraints and Learning}
  \begin{items}
    \begin{oitem}{\litem}
      Main Finding: \\
      \sitem {\footnotesize Skeletal constraints give a significant boost in MOTA and MPJPE.} \\
      \sitem {\footnotesize Both learned Kalman and custom YOLO head improve performance.} \\
      \sitem {\footnotesize Tuning thresholds give a small improvement.}
    \end{oitem}
    \begin{widepage}
      \begin{figure}
        \centering
