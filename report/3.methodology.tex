...
The YOLO26 family of models used for 2D pose estimation in this project perform well at detecting persons and predicting the location of visible keypoints. However, for the integration in the Kalman filter based tracking system there are two major drawbacks of the standard pretrained model. First, the model does not natively output its confidence in the keypoint location predictions. It only outputs a visibility score that represents whether the keypoint is visible or not. While one may naively assume that this score could be transformed into an uncertainty score, this is not without drawbacks since not all visible or invisible keypoints have the same uncertainty. The second limitation of the standard YOLO models is that they no not even attempt to guess at the location of invisible keypoints. Instead, the keypoint locations returned by the model for invisible keypoints are largely not meaningful. However, in many situations it would be possible to make some educated guess about the location of the keypoints, even if with large uncertainty, which would lead to improved tracking performance.
